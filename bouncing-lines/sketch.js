...
function setup() {
  ...
  // 다각형의 변 반지름 선정
  for(let i=0; i < edgs_num - 4; i++){
    edgesRadius.push(random(80, width*0.5))
  }

  //닫힌 다각형을 그리기
  for(let i = 0; i < edgs_num - 4; i++) {
    let angle = i * 2 * PI / (edgs_num - 4)

    let r1 = edgesRadius[i]
    let r2 = i=== (edgs_num-4)-1 ? edgesRadius[0] : edgesRadius[i+1]
    ...
    // second coord
    let x2 = width * 0.5 + r2 * cos(angle + 2* PI / (edgs_num-4))
    let y2 = height * 0.5 + r2 * sin(angle + 2 * PI / (edgs_num-4))

    edges.push(new Edge(x1, y1, x2, y2))
  }
  edges[edgs_num - 1] = new Edge(0, 0, width, 0)
  edges[edgs_num - 2] = new Edge(width, height, 0, height)
  edges[edgs_num - 3] = new Edge(0, height, 0, 0)
