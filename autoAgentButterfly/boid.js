...
class Boid {
  ...
  run(boids) {
    this.maxspeed = sliderSep.value()*0.25;
    this.flock(boids);
    this.update();
    this.borders();
    this.render();
  ...
  flock(boids) {
    ...
    this.applyForce(coh);
    if (frameCount%20 === 0 ) {
      let wind = createVector(map(sin(noise(frameCount)), -1, 1, -0.1, 0.1),
      map(cos(noise(frameCount)), -1, 1, -0.12, 0.06));
